  ...
  public function auth(){
	...
	$cek_admin = $this->M_Login->auth_admin($email,$password);
        ...
        if($cek_user->num_rows() > 0){ 
			...
			if ($this->form_validation->run() == TRUE) {
				if($this->M_Login->login_admin($email, $password) == TRUE){
	...
	public function daftar_admin(){
        ...
        array('required' => 'nama belum terisi'));
		$this->form_validation->set_rules('telp_admin', 'telp_admin', 'trim|required',
        array('required' => 'telp user belum terisi'));
		$this->form_validation->set_rules('email_admin', 'email_admin', 'trim|required',
        array('required' => 'email belum terisi'));
        $this->form_validation->set_rules('pass_admin', 'pass_admin', 'trim|required',
		array('required' => 'Password belum terisi'));
		$this->form_validation->set_rules('alamat_admin', 'alamat_admin', 'trim|required',
        array('required' => 'alamat belum terisi'));
		
		if ($this->form_validation->run() == TRUE ) {
	    	$masuk=$this->M_Login->daftar_admin();
				if($masuk==true){
		    		$this->session->set_flashdata('pesan', 'Daftar Akun berhasil');
    				} else{
		    		$this->session->set_flashdata('gagal', 'Daftar Akun gagal');
				}
		    redirect(base_url('Home/administrasi'), 'refresh');
		} else{
		    $this->session->set_flashdata('pesan', validation_errors());
		    redirect(base_url('Home/administrasi'), 'refresh');
	    }
	...
	public function get_detail_admin($id_admin=''){
		$data_detail=$this->M_Login->get_detail_admin($id_admin);
		echo json_encode($data_detail);
	}

	public function edit_admin(){
        $proses_update=$this->M_Login->edit_admin();
        if($proses_update){
           $this->session->set_flashdata('pesan', 'Update admin berhasil');
        }
        else{
            $this->session->set_flashdata('gagal', 'Update admin gagal');
        }
        redirect(base_url('Home/administrasi'), 'refresh');
	}

	public function hapus_admin($id_admin){
		$hapus = $this->M_Login->hapus_admin($id_admin);
		if($hapus == TRUE){
			$this->session->set_flashdata('pesan', 'Hapus admin berhasil');
		} else {
			$this->session->set_flashdata('gagal', 'Hapus admin gagal');
		}
		redirect(base_url('Home/administrasi'), 'refresh');
	}
